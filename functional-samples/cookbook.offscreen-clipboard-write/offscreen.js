// Once the message has been posted from the service worker, checks are made to confirm the message type and target before proceeding. This is so that the module can easily be adapted into existing workflows where secondary uses for the document (or alternate offscreen documents) might be implemented.
...
// Use the offscreen document's `document` interface to write a new value to the system clipboard.  At the time this demo was created (Jan 2023) the `navigator.clipboard` API requires that the window is focused, but offscreen documents cannot be focused. As such, we have to fall back to `document.execCommand()`.
